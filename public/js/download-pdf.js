...
function downloadPdf() {
    ...
    for (const page of allPages) {
        ...
        pdf.text(`SEO: ${page.scores.seo || "-"}`, x, y);

        y += FontSizesPDF.largeText;
        pdf.text(`GEO: ${page.scores.geo || "-"}`, x, y);

        y += FontSizesPDF.largeText;
        pdf.text(`Velocity: ${page.scores.combined || "-"}`, x, y);
        ...
        failed.map((check) => {
            ...
            pdf.setFontSize(FontSizesPDF.smallText).setFont(
                undefined,
                "normal",
            );
            y = increaseCoordinate(y, FontSizesPDF.smallText, pdf);
            ...
            y = increaseCoordinate(
                y,
                fixDesc.length * FontSizesPDF.smallText,
                pdf,
            );
            ...
            if (check.codeExample) {
                ...
                const codeSnippet = pdf.splitTextToSize(
                    check.codeExample,
                    360 - LineBreakPDF.section,
                );
        ...
        passed.map((check) => {
            ...
            y = increaseCoordinate(
                y,
                passDetail.length * FontSizesPDF.smallText,
                pdf,
            );
        });
    }
