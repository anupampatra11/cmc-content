...
function renderAiSuggestions(ai) {
    ...
    if (!highPriority.length && !claudeOnly.length && !openaiOnly.length)
        return "";
    ...
    if (highPriority.length) {
        ...
        html += highPriority
            .map(({ claude }, i) => renderAiSuggestionItem(claude, i, "hp"))
            .join("");
    }
    ...
    if (claudeOnly.length) {
        ...
        html += claudeOnly
            .map((s, i) => renderAiSuggestionItem(s, i, "cl"))
            .join("");
    }
    ...
    if (openaiOnly.length) {
        ...
        html += openaiOnly
            .map((s, i) => renderAiSuggestionItem(s, i, "oi"))
            .join("");
    }
...
function showResults(data) {
    ...
    if (allPages.length > 0) selectPage(indexHighestRankedPage);
}
...
function renderPageList() {
    ...
    sortedPages.forEach((page) => {
        ...
        div.className =
            "page-item" + (origIdx === selectedPage ? " active" : "");
