...
function checkWordCount(page) {
    ...
    return check(
        ...
        wordPass,
        wordPass
            ? `${page.wordCount} words \u2014 good depth`
            : `Only ${page.wordCount} words \u2014 too thin for good rankings`,
        7,
...
function checkCanonicalTag(page) {
    // 7. Canonical tag
    ...
    return check(
        ...
        canonicalPass,
        canonicalPass
            ? `Canonical: ${truncate(page.canonicalUrl, 60)}`
...
function checkSchemaMarkup(page) {
    // 8. Schema markup
    ...
    return check(
        ...
        schemaPass,
        schemaPass
            ? `Found: ${page.schemaTypes.join(", ")}`
            : "No JSON-LD schema \u2014 critical for rich results and GEO",
        10,
...
function checkInternalLinks(page) {
    ...
    return check(
        ...
        internalPass,
        internalPass
            ? `${page.internalLinkCount} internal links`
            : `Only ${page.internalLinkCount} internal link(s) \u2014 add more for crawlability`,
        7,
...
function checkCleanUrl(page) {
    // 10. URL structure
    const cleanUrl =
        safe(page.slug).length < 80 && !safe(page.slug).includes("?");
    return check(
        "url",
        "Technical",
        "Clean URL structure",
        ...
        cleanUrl
            ? `Clean URL: ${page.slug}`
            : "URL has query parameters or is too long \u2014 use clean, descriptive slugs",
        5,
...
function checkAuthor(page) {
    ...
    return check(
        ...
        authorPass
            ...
            : "No author field \u2014 critical for E-E-A-T and AI engine citability",
        8,
...
function checkPublishDate(page) {
...
function checkFaqSchema(page) {
    ...
    return check(
        ...
        hasFaq,
        hasFaq
            ? "FAQPage schema found \u2014 excellent GEO signal"
            : "No FAQ schema \u2014 adding Q&A markup significantly boosts AI citability",
        8,
