...
function mergeSuggestions(claudeSuggestions, openaiSuggestions) {
    ...
    for (const cs of claudeSuggestions) {
        ...
        if (bestScore >= 0.25 && bestOiIdx >= 0) {
            highPriority.push({
                claude: cs,
                openai: openaiSuggestions[bestOiIdx],
            });
    ...
    const openaiOnly = openaiSuggestions.filter(
        (_, idx) => !openaiMatched.has(idx),
    );
...
function mergeAiScores(claude, openai) {
    ...
    const merged =
        claude && openai
            ...
            : {
                  highPriority: [],
                  claudeOnly: claudeSugg,
                  openaiOnly: openaiSugg,
              };
...
function updateAverages(result) {
    ...
    result.avgSeo = Math.round(
        completed.reduce((s, p) => s + p.scores.seo, 0) / completed.length,
    );
    result.avgGeo = Math.round(
        completed.reduce((s, p) => s + p.scores.geo, 0) / completed.length,
    );
    result.avgCombined = Math.round(
        completed.reduce((s, p) => s + p.scores.combined, 0) / completed.length,
    );
}
