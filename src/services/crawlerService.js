...
const FACTUAL_PATTERN =
    /\d+([,.]\d+)?(%|\s*(percent|million|billion|thousand))?|\b(according to|research|study|studies|report|survey|data|statistics|found that)\b/gi;
...
async function crawl(url) {
    ...
    // Schema field counts
    const { validSchemaFields, totalSchemaFields } =
        extractSchemaFieldCounts($);

    // Author
    const authorName = extractAuthor($);

    // Dates
    const datePublished = extractMeta($, [
        "article:published_time",
        "datePublished",
    ]);
    const dateModified = extractMeta($, [
        "article:modified_time",
        "dateModified",
    ]);
    const daysSinceLastUpdate = computeDaysSinceUpdate(
        dateModified,
        datePublished,
    );
    ...
    const metaDescHtml =
        $.html($('meta[name="description"]').first()) ||
    ...
    const avgChunkSize =
        headingCount > 0 ? totalWords / headingCount : totalWords;
    const technicalTermCount = countTechnicalTerms(bodyText);

    console.log(
        `Crawled: ${url} | words: ${wordCount} | schema: ${JSON.stringify(schemaTypes)}`,
    );
...
function extractSchemaFieldCounts($) {
    let validSchemaFields = 0,
        totalSchemaFields = 0;
    $('script[type="application/ld+json"]').each((_, el) => {
        try {
            const root = JSON.parse($(el).html() || "{}");
            const schemas =
                root["@graph"] && Array.isArray(root["@graph"])
                    ? root["@graph"]
                    : [root];
            for (const schema of schemas) {
                const type = schema["@type"] || "";
                totalSchemaFields += SCHEMA_EXPECTED_FIELDS[type] || 5;
                validSchemaFields += countNonEmptySchemaFields(
                    schema,
                    new Set(),
                );
            }
...
function countNonEmptySchemaFields(node, counted) {
    ...
    for (const [key, val] of Object.entries(node)) {
        ...
        if (typeof val === "object")
            count += countNonEmptySchemaFields(val, counted);
    }
    return count;
}
...
function extractAuthor($) {
    ...
    // Try selectors
    const authorEl = $(
        '[rel="author"], .author-name, .byline, [itemprop="author"]',
    ).first();
    if (authorEl.length) {
...
function extractMeta($, names) {
    for (const name of names) {
        ...
        const datetime =
            timeEl.attr("datetime") || timeEl.attr("content") || "";
...
function countMeaningfulWords(text) {
    ...
    return text
        .split(/\s+/)
        .filter((t) => t.replace(/[^a-zA-Z]/g, "").length > 0).length;
}

function countSentences(text) {
    if (!text) return 0;
    return text.split(/[.?!]+/).filter((s) => s.trim().split(/\s+/).length >= 3)
        .length;
}
